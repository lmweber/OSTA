...
To align Xenium and Visium, we use the affine transformation matrix provided by 
10x Genomics, which was obtained by registering Xenium onto Visium in Python 
with Fiji [@10xGenomics2023-Fiji] Java plug-in.
...
.vis <- vis[gs, cs]
.xem <- xem[gs, cs]
.xem$array_col <- .vis$array_col
.xem$array_row <- .vis$array_row
...
# offset the spatial location for clustering in Xenium pseudo-spot
.xem$array_col <- 100 + .xem$array_col
...
```{r clu, message=FALSE}
# 'BayesSpace' clustering with c("array_row", "array_col") in colData()
...
aggregated Xenium data appears to yield slightly higher granularity, when using 
the default `BayesSpace::spatialCluster(burn.in=1000, nrep=1e4)` parameters:

```{r plt-clu, collapse=TRUE, fig.width=4, fig.height=2.5}
# plot use spatialCoord()
plotVisium(res,
