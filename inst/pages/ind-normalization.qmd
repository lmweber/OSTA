...
::: {.callout-note icon="false" title="Log-library size normalization" collapse="true"}
...
Specifically, denoting with $y_{ij}$ the count of gene $j$ in cell $i$, with $y_{i.}$ the sum of all gene counts for cell $i$ (*library size*), and with $y_{..}$ the sum of all counts across genes and cells, we define a given cell's size factor as
$$\text{sf}_i = \frac{y_{i.}}{y_{..}/n}.$$

where $y_{..}/n$ corresponds to the average library size.
