...
Seq-based ST data can contain zero to multiple cells per spot, which might be fully covered or fractured, depending on the spatial resolution of the platform and the tissue cell density (c.f. @sec-seq-introduction). This aspect of the data necessitates a computational estimation of cell type composition for each spot to analyze the spatial distribution of biological signals. In addition, the spatial coordinates of ST data sets it apart from single-cell RNA sequencing data, making it sub-optimal to directly apply existing single-cell label transfer methods.

```{r, echo = FALSE, out.width = "33%", fig.cap = "Schematic illustrating multiple cells in a 10x Visium spot (diameter of 55um; purple line) overlaying the H&E image. The distance between spots is 100um (yellow line), and an average immune cells' diameter is around 10um (cyan line)."}
knitr::include_graphics("../images/seq-deconvolution.png")
```

There has been around twenty deconvolution techniques proposed for spot-level ST. Some methods require borrowing insights from a scRNAseq reference, while others can be reference-free. Based on their underlying algorithms, @li2023comprehensive have grouped methods into five categories:

- **probabilistic-based**: use Bayesian inference, likelihood estimation, or probabilistic modeling to estimate cell-type compositions while incorporating uncertainty. Available tools are `r BiocStyle::Githubpkg("dmcable/spacexr")` (i.e. `RCTD`), `r BiocStyle::Githubpkg("YMa-lab/CARD")`, `r BiocStyle::Biocpkg("SpatialDecon")`, and `r BiocStyle::Githubpkg("JEFworks-Lab/STdeconvolve")` in R, and `r BiocStyle::Githubpkg("SpatialTranscriptomicsResearch/std-poisson")` (i.e. `Berglund et al.`), `r BiocStyle::Githubpkg("almaan/stereoscope")`, `r BiocStyle::Githubpkg("scverse/scvi-tools")` (i.e. `DestVI`), `r BiocStyle::Githubpkg("DongqingSun96/STRIDE")`, and `r BiocStyle::Githubpkg("BayraktarLab/cell2location")` in Python.
- **non-negative matrix factorization (NMF)-based**: decompose gene expression data into latent components representing different cell types. Available tools are `r BiocStyle::Githubpkg("RubD/Giotto")` (i.e. `SpatialDWLS`) and `r BiocStyle::Biocpkg("SPOTlight")` in R, and [NMFReg](https://github.com/tudaga/NMFreg_tutorial) in Python.
- **graph-based**: use graph neural networks or graph-based optimization to model spatial relationships. Available tools are `r BiocStyle::Githubpkg("leihouyeung/SD2")` in R, and `r BiocStyle::Githubpkg("Su-informatics-lab/DSTG")` and `r BiocStyle::Githubpkg("ma-compbio/SpiceMIx")` in Python.
- **optimal transport (OT)-based**: infer spatial gene expression distributions by mapping scRNAseq and ST data. Available tools are `r BiocStyle::Githubpkg("zcang/SpaOTsc")` and `r BiocStyle::Githubpkg("rajewsky-lab/novosparc")` in Python.
- **deep learning-based**: align and integrate single-cell and spatial transcriptomic data with neural networks. For example, `r BiocStyle::Githubpkg("broadinstitute/Tangram")` in Python.
...
xs <- range(xy[, 1]); ys <- nrow(imgRaster(vis))-range(xy[, 2])
box <- geom_rect(
    xmin=xs[1], xmax=xs[2], ymin=ys[1], ymax=ys[2],
    col="black", fill=NA, linetype=2, linewidth=2/3)
plotVisium(vis, spots=FALSE, point_size=.75) + box +
plotVisium(vis, point_size=1.5, zoom=TRUE) +
plot_layout(nrow=1) & facet_null()
```

Deconvolution comes after proper quality control, as detailed in @sec-seq-quality-control, and is usually performed on unnormalized and untransformed (i.e., raw) counts. Here, we quickly check if any spot has a count of 0 and remove it.
...
plotSpots(vis, annotate="log_sum") + ggtitle("log-library size") +
plotSpots(vis, annotate="subsets_mt_percent") + ggtitle("% mitochondrial") +
...
Next, we perform deconvolution with `r BiocStyle::Githubpkg("dmcable/spacexr")` (f.k.a. RCTD)[@Cable2022-RCTD].
[Note that RCTD can also be adapted to VisiumHD data with `rctd_mode="doublet"`, as demonstrated by [@Oliveira2024] and @sec-seq-workflow-visium-hd.]{.aside}
...
```{r plt-dec-xy, message=FALSE, fig.width=10, fig.height=5}
#| code-fold: true
lapply(names(ws), \(.) 
...
For comparison with spot annotations provided by 10x Genomics, we include majority voted cell type from deconvolution as `RCTD`.
...
Note the strong stromal signals and macrophage being the second common cell type for stromal cells.
...
Overall, we observe agreement between the provided spot-labels and `RCTD` deconvolution derived annotations.
...
- @li2023comprehensive benchmarked 18 deconvolution methods, including `RCTD`, across 50 simulated and real datasets. Among these methods, `CARD`, `cell2location`, and `Tangram` are highly recommended. Figure 1 summarizes the methods performance, and Figure 4 gives a flowchart of how to decide on which method to use.
